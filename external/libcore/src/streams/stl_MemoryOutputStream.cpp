...
namespace stl {
    ...
    bool MemoryOutputStream::appendUTF8Char(stl_wchar c) {
    ...
    int64 MemoryOutputStream::seek(int64 newPosition) {
        setPosition(newPosition);
        return newPosition;
    }

    void MemoryOutputStream::alignTo(const size_t alignment) {
        flush();
        auto real_size = RoundUp(size, alignment);
        if (real_size - size > 0) {
            seek(size);
            auto buffer_length = real_size - size;

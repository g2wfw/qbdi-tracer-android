...
namespace stl {
    StreamProcess::Data::Data() noexcept: id(0) {
    }
    ...
    static stl::String join_arguments(const StringArray &args) {
        using charset = CharSet<char>;

        std::string ret;

        for (const auto &arg_: args) {
            auto arg = arg_.string();
            if (!ret.empty())
    ...
    StreamProcess::id_type StreamProcess::open(const String &command, const String &path,
        ...
        DWORD creation_flags = stdin_fd || stdout_fd || stderr_fd ? CREATE_NO_WINDOW : 0;
        // CREATE_NO_WINDOW cannot be used when stdout or stderr is redirected to parent process
        String environment_str;
        if (environment) {
#ifdef UNICODE
            for (const auto &e: *environment)
                environment_str += e.first + L'=' + e.second + L'\0';
            if (!environment_str.isEmpty())
                environment_str += L'\0';
        ...
        char *process_command_str_ = const_cast<char *>(process_command.isEmpty() ? nullptr : process_command.c_str());
        char *environment_str_ = const_cast<char *>(environment_str.isEmpty() ? nullptr : environment_str.c_str());

        BOOL bSuccess = CreateProcessA(nullptr,
                                       process_command_str_, nullptr, nullptr,
                                       stdin_fd || stdout_fd || stderr_fd || config.inherit_file_descriptors,
                // Cannot be false when stdout, stderr or stdin is used
                                       creation_flags,
                                       environment_str_,
                                       path.isEmpty() ? nullptr : path.c_str(),
            ...
            stdout_thread = std::thread([this]() {
                ...
                for (;;) {
                    BOOL bSuccess = ReadFile(*stdout_fd, static_cast<CHAR *>(buffer.get()),
                                             static_cast<DWORD>(config.buffer_size), &n, nullptr);
                    if (!bSuccess || n == 0) {

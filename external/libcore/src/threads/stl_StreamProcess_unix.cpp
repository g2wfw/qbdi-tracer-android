...
namespace stl {
  ...
  return -1;
    ...
    StreamProcess::Data::Data() noexcept : id(-1) {
    }


    StreamProcess::id_type StreamProcess::open(const std::function<void()> &function) noexcept {
        if (open_stdin)
    ...
    StreamProcess::id_type StreamProcess::open(const StringArray &arguments, const String &path,
        ...
        return open([this, &arguments, &path, &environment] {
            ...
            if (!environment)
                ...
                portable_execvpe(argv_ptrs[0], const_cast<char *const *>(argv_ptrs.data()),
                                 const_cast<char *const *>(env_ptrs.data()));
            }
        });
    }

    StreamProcess::id_type StreamProcess::open(const String &command, const String &path,
                                               const environment_type *environment) noexcept {
        return open([this, &command, &path, &environment] {
            auto command_c_str = command.c_str();
            stl::String cd_path_and_command;

            if (!path.isEmpty()) {
                std::string path_escaped = path.string();
                size_t pos = 0;
                ...
                cd_path_and_command = "cd '" + path_escaped + "' && "; // To avoid resolving symbolic links
                cd_path_and_command.append(command);
            ...
            if (!environment) {
                if (config.flatpak_spawn_host)
                    // break out of sandbox, execute on host
                    execl("/usr/bin/flatpak-spawn", "/usr/bin/flatpak-spawn", "--host", "/bin/sh", "-c", command_c_str,
                          nullptr);
                else
                ...
                if (config.flatpak_spawn_host)
                    // break out of sandbox, execute on host
                    execle("/usr/bin/flatpak-spawn", "/usr/bin/flatpak-spawn", "--host", "/bin/sh", "-c", command_c_str,
                           nullptr, env_ptrs.data());
                else
    ...
    void StreamProcess::async_read() noexcept {
        ...
        stdout_stderr_thread = std::thread([this] {
            ...
            if (stdout_fd) {
                ...
                pollfds.back().fd = fcntl(*stdout_fd, F_SETFL, fcntl(*stdout_fd, F_GETFL) | O_NONBLOCK) == 0
                                        ? *stdout_fd
                                        : -1;
            ...
            if (stderr_fd) {
                pollfds.emplace_back();
                pollfds.back().fd = fcntl(*stderr_fd, F_SETFL, fcntl(*stderr_fd, F_GETFL) | O_NONBLOCK) == 0
                                        ? *stderr_fd
                                        : -1;
                pollfds.back().events = POLLIN;
            }
    ...
    int StreamProcess::get_exit_status() noexcept {
        if (data.id <= 0)
            return -1;
    ...
    bool StreamProcess::try_get_exit_status(int &exit_status) noexcept {
        if (data.id <= 0) {
            exit_status = -1;
            return true;
        }
    ...
    void StreamProcess::close_fds() noexcept {
    ...
    bool StreamProcess::write(const char *bytes, size_t n) {
        if (!open_stdin)
    ...
    void StreamProcess::close_stdin() noexcept {
        std::lock_guard<std::mutex> lock(stdin_mutex);
        if (stdin_fd) {
            if (data.id > 0)
                close(*stdin_fd);
    ...
    void StreamProcess::kill(bool force) noexcept {
        std::lock_guard<std::mutex> lock(close_mutex);
        if (data.id > 0 && !closed) {
            if (force) {
                ::kill(-data.id, SIGTERM);
                ::kill(data.id, SIGTERM);
                // Based on comment in https://gitlab.com/eidheim/tiny-process-library/-/merge_requests/29#note_1146144166
            } else {
                ::kill(-data.id, SIGINT);
                ::kill(data.id, SIGINT);
            }
    ...
    void StreamProcess::kill(id_type id, bool force) noexcept {
        if (id <= 0)
            return;
    ...
    void StreamProcess::signal(int signum) noexcept {
        std::lock_guard<std::mutex> lock(close_mutex);
        if (data.id > 0 && !closed) {
            ::kill(-data.id, signum);
            ::kill(data.id, signum);

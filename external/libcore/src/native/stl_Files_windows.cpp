...
//==============================================================================
    namespace WindowsFileHelpers {
        //==============================================================================
#if STL_WINDOWS
        STL_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wnested-anon-types")
    ...
    void MemoryMappedFile::openInternal(const File &file, AccessMode mode, bool exclusive) {
        ...
        auto h = CreateFile(file.getFullPathName().toWideCharPointer(), accessMode,
                            exclusive ? 0 : (FILE_SHARE_READ | FILE_SHARE_DELETE |
                                             (mode == readWrite ? FILE_SHARE_WRITE : 0)), nullptr,
...
//==============================================================================
    File STL_CALLTYPE File::getSpecialLocation(const SpecialLocationType type) {
        int csidlType = 0;
                    ...
                    if (IsReparseTagMicrosoft (reparseData->ReparseTag)) {
                        ...
                        switch (reparseData->ReparseTag) {
                            case IO_REPARSE_TAG_SYMLINK: {
                                ...
                                targetPath = {
                                        symlinkData.PathBuffer + (symlinkData.SubstituteNameOffset / sizeof(WCHAR)),
...
//==============================================================================
    ...
    public:
        Pimpl(const File &directory, const String &wildCardIn)
                ...
                directory.getFullPathName().isNotEmpty() ? File::addTrailingSeparator(directory.getFullPathName()) +
                                                           wildCardIn : String()),
    ...
    private:
        ...
        STL_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Pimpl)
    };
...
//==============================================================================
    bool STL_CALLTYPE Process::openDocument(const String &fileName, const String &parameters) {
    ...
    void File::revealToUser() const {
        ...
        STL_LOAD_WINAPI_FUNCTION (dll, ILCreateFromPathW, ilCreateFromPathW, ITEMIDLIST*, (LPCWSTR))
        STL_LOAD_WINAPI_FUNCTION (dll, ILFree, ilFree, void, (ITEMIDLIST * ))
        STL_LOAD_WINAPI_FUNCTION (dll, SHOpenFolderAndSelectItems, shOpenFolderAndSelectItems, HRESULT,

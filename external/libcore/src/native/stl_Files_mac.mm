...
#import "native/stl_CFHelpers_mac.h"
#import "native/stl_ObjCHelpers_mac.h"
#include "system/stl_Platform.h"
namespace stl {

/*
    Note that a lot of methods that you'd expect to find in this file actually
    live in stl_posix_SharedCode.h!
*/
            ...
            switch (type) {
                case userHomeDirectory:
                    ...
                    case tempDirectory:
                    {
                        ...
                        tmp = tmp.getChildFile (stl_getExecutableFile().getFileNameWithoutExtension());
                        tmp.createDirectory();
                ...
                case tempDirectory: {
                    File tmp("~/Library/Caches/" + stl_getExecutableFile().getFileNameWithoutExtension());
                    tmp.createDirectory();
                ...
                case invokedExecutableFile:
                    if (stl_argv != nullptr && stl_argc > 0)
                        return File::getCurrentWorkingDirectory().getChildFile(String(CharPointer_UTF8(stl_argv[0])));
                ...
                case currentApplicationFile: {
                    const File exe(stl_getExecutableFile());
                ...
                case currentExecutableDirectory: {
                    ...
                    _NSGetExecutablePath(buffer.get(), &size);
                    auto file = File(String::fromUTF8(buffer, (int) size));
...
//==============================================================================
    ...
    public:
        Pimpl(const File &directory, const String &wildcard)
                : parentDir(File::addTrailingSeparator(directory.getFullPathName())),
                  wildCard(wildcard) {
            STL_AUTORELEASEPOOL {
                enumerator = [[[NSFileManager defaultManager] enumeratorAtPath:juceStringToNS(
                        directory.getFullPathName())] retain];
            }
        }
    ...
    private:
        ...
        STL_DECLARE_NON_COPYABLE (Pimpl)
    };
    ...
    bool DirectoryIterator::NativeIterator::next(String &filenameFound,
                                                 bool *const isDir, bool *const isHidden, int64 *const fileSize,
                                                 Time *const modTime, Time *const creationTime,
...
//==============================================================================
    bool STL_CALLTYPE Process::openDocument(const String &fileName, [[maybe_unused]] const String &parameters) {
        STL_AUTORELEASEPOOL {
            NSString *fileNameAsNS(juceStringToNS(fileName));
            NSURL *filenameAsURL = File::createFileWithoutCheckingPath(fileName).exists()
...
#if !STL_IOS
        if (exists())
            [[NSWorkspace sharedWorkspace] selectFile:juceStringToNS(
                    getFullPathName())     inFileViewerRootedAtPath:nsEmptyString()];
...
//==============================================================================
    OSType File::getMacOSType() const {
        STL_AUTORELEASEPOOL {
            NSDictionary *fileDict = [[NSFileManager defaultManager] attributesOfItemAtPath:juceStringToNS(
                    getFullPathName())                               error:nil];
    ...
    void File::addToDock() const {
        ...
        if (!stl_getOutputFromCommand("defaults read com.apple.dock persistent-apps").containsIgnoreCase(
                getFullPathName())) {
            stl_runSystemCommand(
            ...
            stl_runSystemCommand("osascript -e \"tell application \\\"Dock\\\" to quit\"");
        }
    }
    ...
    File File::getContainerForSecurityApplicationGroupIdentifier(const String &appGroup) {
        if (@available(macOS 10.8, *))
            if (auto *url = [[NSFileManager defaultManager] containerURLForSecurityApplicationGroupIdentifier:juceStringToNS(
                    appGroup)])

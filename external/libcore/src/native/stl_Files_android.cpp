...
private:
    ...
    static Array<File> getSecondaryStorageDirectories()
    {
        ...
        else
        {
            ...
            stl_statStruct info;
            if (! stl_stat (mountFolder.getFullPathName(), info))
                return {};
            ...
            for (const auto& iter : RangedDirectoryIterator (mountFolder, false, "*", File::findDirectories))
            {
                ...
                if (stl_stat (candidate.getFullPathName(), info)
    ...
    static File getMountPointForFile (const File& file)
    {
        ...
        if (stl_stat (file.getFullPathName(), info))
        {
            ...
            for (;;)
            {
                ...
                stl_stat (parent.getFullPathName(), info);
...
bool File::moveToTrash() const
{
 ...
 bool STL_CALLTYPE Process::openDocument (const String& fileName, const String&)
{

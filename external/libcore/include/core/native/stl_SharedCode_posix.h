...
//==============================================================================
    void STL_CALLTYPE Thread::sleep(int millisecs) {
...
#ifndef STL_PREFERRED_MAX_FILE_HANDLES
            enum {
                STL_PREFERRED_MAX_FILE_HANDLES = 8192
            };
#endif

            // Try to give our app a decent number of file handles by default
            if (!Process::setMaxNumberOfFileHandles(0)) {
                for (int num = STL_PREFERRED_MAX_FILE_HANDLES; num > 256; num -= 1024)
...
//==============================================================================
    ...
    const stl_wchar File::separator = '/';
    ...
    stl_wchar File::getSeparatorChar() { return '/'; }
...
// The unix siginterrupt function is deprecated - this does the same job.
    int stl_siginterrupt([[maybe_unused]] int sig, [[maybe_unused]] int flag) {
#if STL_WASM
        return 0;
#else
#if STL_ANDROID
        using stl_sigactionflags_type = unsigned long;
#else
        using stl_sigactionflags_type = int;
#endif
        ...
        if (flag != 0)
            act.sa_flags &= static_cast<stl_sigactionflags_type> (~SA_RESTART);
        else
            act.sa_flags |= static_cast<stl_sigactionflags_type> (SA_RESTART);
...
//==============================================================================
    namespace {
#if STL_LINUX || (STL_IOS && (!TARGET_OS_MACCATALYST) && (!__DARWIN_ONLY_64_BIT_INO_T)) // (this iOS stuff is to avoid a simulator bug)
        using stl_statStruct = struct stat64;
#define STL_STAT  stat64
#else
        using stl_statStruct = struct stat;
#define STL_STAT  stat
#endif

        bool stl_stat(const String &fileName, stl_statStruct &info) {
            return fileName.isNotEmpty()
                   && STL_STAT(fileName.toUTF8(), &info) == 0;
        }

#if !STL_WASM

        // if this file doesn't exist, find a parent of it that does..
        bool stl_doStatFS(File f, struct statfs &result) {
            for (int i = 5; --i >= 0;) {
                if (f.exists())
                    break;
        ...
        static int64 getCreationTime(const stl_statStruct &s) noexcept { return (int64) s.st_birthtime; }

#else

        static int64 getCreationTime(const stl_statStruct &s) noexcept { return (int64) s.st_ctime; }
        ...
        void updateStatInfoForFile(const String &path, bool *isDir, int64 *fileSize,
            ...
            if (isDir != nullptr || fileSize != nullptr || modTime != nullptr || creationTime != nullptr) {
                stl_statStruct info;
                const bool statOk = stl_stat(path, info);
    ...
    bool File::isDirectory() const {
        ...
        return fullPath.isNotEmpty()
               && (stl_stat(fullPath, info) && ((info.st_mode & S_IFDIR) != 0));
    }
    ...
    int64 File::getSize() const {
        stl_statStruct info;
        return stl_stat(fullPath, info) ? info.st_size : 0;
    }

    uint64 File::getFileIdentifier() const {
        stl_statStruct info;
        return stl_stat(fullPath, info) ? (uint64) info.st_ino : 0;
    }
...
//==============================================================================
    ...
    void File::getFileTimesInternal(int64 &modificationTime, int64 &accessTime, int64 &creationTime) const {
        ...
        if (stl_stat(fullPath, info)) {
#if STL_MAC || (STL_IOS && __DARWIN_ONLY_64_BIT_INO_T)
    ...
    bool File::setFileTimesInternal(int64 modificationTime, int64 accessTime, int64 /*creationTime*/) const {
        ...
        if ((modificationTime != 0 || accessTime != 0) && stl_stat(fullPath, info)) {
#if STL_MAC || (STL_IOS && __DARWIN_ONLY_64_BIT_INO_T)
            ...
            times[1].tv_usec = setModificationTime ? static_cast<__darwin_suseconds_t> ((modificationTime % 1000) *
                                                                                        1000)
                                                   : static_cast<__darwin_suseconds_t> (info.st_mtimespec.tv_nsec /
                                                                                        1000);
            ...
            times.actime =
                    accessTime != 0 ? static_cast<time_t> (accessTime / 1000) : static_cast<time_t> (info.st_atime);
            times.modtime = modificationTime != 0 ? static_cast<time_t> (modificationTime / 1000)
    ...
    File stl_getExecutableFile() {
        struct DLAddrReader {
            static String getFilename() {
                ...
                auto localSymbol = (void *) stl_getExecutableFile;
...
//==============================================================================
    ...
    void stl_runSystemCommand(const String &);

    void stl_runSystemCommand(const String &command) {
        [[maybe_unused]] int result = system(command.toUTF8());
    }

    String stl_getOutputFromCommand(const String &);

    String stl_getOutputFromCommand(const String &command) {
        ...
        stl_runSystemCommand(command + " > " + tempFile.getFullPathName());
...
//==============================================================================
    ...
    void STL_CALLTYPE Thread::setCurrentThreadName(const String &name) {
#if STL_IOS || STL_MAC
        pthread_setname_np(name.toRawUTF8());
#elif STL_LINUX || STL_BSD || STL_ANDROID
#if (STL_BSD \
 || (STL_LINUX && (__GLIBC__ * 1000 + __GLIBC_MINOR__) >= 2012) \
 || (STL_ANDROID && __ANDROID_API__ >= 9))
    ...
    void STL_CALLTYPE Thread::yield() {
        sched_yield();
    }
...
#define SUPPORT_AFFINITIES 1
    ...
    void STL_CALLTYPE Thread::setCurrentThreadAffinityMask([[maybe_unused]] uint32 affinityMask) {
        ...
        for (int i = 0; i < 32; ++i) {
            if ((affinityMask & (uint32) (1 << i)) != 0) {
                ...
                STL_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wsign-conversion")
                CPU_SET ((size_t) i, &affinity);
                STL_END_IGNORE_WARNINGS_GCC_LIKE
            }
        }

#if (!STL_ANDROID) && ((!(STL_LINUX || STL_BSD)) || ((__GLIBC__ * 1000 + __GLIBC_MINOR__) >= 2004))

...
private:
    ...
    {
        return root + String::toHexString (stl::Random::getSystemRandom().nextInt64());
    }

    STL_DECLARE_NON_COPYABLE (ObjCClass)
};
...
struct ObjCLifetimeManagedClass : public ObjCClass<NSObject>
{
        ...
        : ObjCClass<NSObject> ("ObjCLifetimeManagedClass_")
    {
        ...
        STL_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wundeclared-selector")
...
// this will return an NSObject which takes ownership of the STL instance passed-in
...
{
    STL_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wobjc-method-access")
    return [ObjCLifetimeManagedClass<Class>::objCLifetimeManagedClass.createInstance() initWithJuceObject:obj];
    STL_END_IGNORE_WARNINGS_GCC_LIKE
}

// Get the STL class instance that was tied to the life-time of an NSObject with the
// function above
template <typename Class>

...
namespace stl
{
...
#if (STL_MINGW && STL_32BIT) || (! defined (_MSC_VER) && ! defined (__uuidof))
 #ifdef __uuidof
  #undef __uuidof
 #endif
 ...
 #define STL_DECLARE_UUID_GETTER(name, uuid) \
    template <> struct UUIDGetter<name> { static CLSID get()  { return uuidFromString (uuid); } };

 #define STL_COMCLASS(name, guid) \
    struct name; \
    STL_DECLARE_UUID_GETTER (name, guid) \
    struct name

#else
 #define STL_DECLARE_UUID_GETTER(name, uuid)
 #define STL_COMCLASS(name, guid)       struct DECLSPEC_UUID (guid) name
#endif

#define STL_IUNKNOWNCLASS(name, guid)   STL_COMCLASS(name, guid) : public IUnknown
#define STL_COMRESULT                   HRESULT STDMETHODCALLTYPE
#define STL_COMCALL                     virtual HRESULT STDMETHODCALLTYPE

STL_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wlanguage-extension-token")
...
protected:
    ...
    STL_COMRESULT QueryInterface (REFIID refId, void** result)
    {
    ...
    template <class Type>
    STL_COMRESULT castToType (void** result)
    {
        this->AddRef();

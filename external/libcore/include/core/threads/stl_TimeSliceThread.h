...
/**
  由TimeSliceThread类使用。
若要使用TimeSliceThread注册类，请从此类派生并
使用TimeSliceThread:：addTimeSliceClient（）方法将其添加到列表中。
请确保在之前始终调用TimeSliceThread:：removeTimeSliceClient（）
删除您的客户端！
@请参阅TimeSliceThread
@标签｛Core｝
*/
    class STL_EXPORT TimeSliceClient {
    public:
        /** Destructor. */
...
*/
    ...
    private:
        ...
        STL_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (TimeSliceThread)
    };

...
#include <optional>
#include <functional>
#include "stl_CriticalSection.h"
#include "stl_WaitableEvent.h"
...
*/
    ...
    public:
        ...
        struct RealtimeOptions {
            ...
            */
            [[nodiscard]] RealtimeOptions withPriority(int newPriority) const {
                ...
                return withMember(*this, &RealtimeOptions::priority, stl::jlimit(0, 10, newPriority));
            }
            ...
            */
            ...
            withApproximateAudioProcessingTime(int samplesPerFrame, double sampleRate) const {
        ...
        */
        ...
        //==============================================================================
        /** 使用自定义优先级在自己的线程上调用lambda或函数。
            这将启动一个Thread对象，该对象调用该函数，然后退出。
            请记住，启动和停止线程可能是一项相当艰巨的任务
            操作，所以如果你经常启动，你可能更喜欢使用ThreadPool
            简短的背景任务。
            还要注意，使用匿名线程会使中断变得非常困难
            当你需要停止它时的功能，例如当你的应用程序退出时。所以这取决于
            您可以处理函数可能无法及时停止的情况。
        ...
        */
        ...
        /** Changes the affinity mask for the caller thread.
            ...
            @see setAffinityMask
        */
        static void STL_CALLTYPE setCurrentThreadAffinityMask(uint32 affinityMask);
        ...
        */
        static void STL_CALLTYPE sleep(int milliseconds);
        ...
        */
        static void STL_CALLTYPE yield();
        ...
        */
        ...
        /** Returns an id that identifies the caller thread.
            ...
            @see getThreadId
        */
        static ThreadID STL_CALLTYPE getCurrentThreadId();

        /** Finds the thread object that is currently running.

            Note that the main UI thread (or other non-STL threads) don't have a Thread
            object associated with them, so this will return nullptr.
        */
        static Thread *STL_CALLTYPE getCurrentThread();
        ...
        */
        static void STL_CALLTYPE setCurrentThreadName(const String &newThreadName);

#if STL_ANDROID || DOXYGEN
        //==============================================================================
        /** Initialises the STL subsystem for projects not created by the Projucer

            On Android, STL needs to be initialised once before it is used. The Projucer
            will automatically generate the necessary java code to do this. However, if
            you are using STL without the Projucer or are creating a library made with
            STL intended for use in non-STL apks, then you must call this method
            ...
            @code
            com.rmsl.juce.Java.initialiseSTL (myContext);
            @endcode
            ...
            public class Java
            {
                static { System.loadLibrary ("stl_jni"); }
                public native static void initialiseSTL (Context context);
            }
            @endcode
            ...
            @param jContext this is a jobject referring to your app/service/receiver/
                            provider's Context. STL needs this for many of it's internal
                            functions.
        */
        static void initialiseSTL (void* jniEnv, void* jContext);
#endif
    ...
    private:
...
#if STL_ANDROID || STL_LINUX || STL_BSD
        std::atomic<Priority> priority;
#endif

#ifndef DOXYGEN

        friend void stl_threadEntryPoint(void *);
        ...
        STL_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (Thread)
    };

...
struct local_alloc_t
{
    ...
    STL_LOG_CONSTEXPR local_alloc_t() STL_LOG_NOEXCEPT : hlocal_(nullptr) {}

    local_alloc_t(local_alloc_t const &) = delete;
    local_alloc_t &operator=(local_alloc_t const &) = delete;

    ~local_alloc_t() STL_LOG_NOEXCEPT
    {
        if (hlocal_)
        {
            LocalFree(hlocal_);
...
public:
    ...
    static sid_t duplicate_sid(PSID psid)
    {
        ...
        {
            STL_LOG_THROW(win32_error("CopySid"));
        }
    ...
    static sid_t get_current_user_sid()
    {
        ...
        struct process_token_t
        {
            ...
            explicit process_token_t(HANDLE process)
            {
                ...
                {
                    STL_LOG_THROW(win32_error("OpenProcessToken"));
                }
            }
        ...
        {
            STL_LOG_THROW(win32_error("GetTokenInformation should fail"));
        }
        ...
        {
            STL_LOG_THROW(win32_error("GetTokenInformation"));
        }
...
private:
    ...
    HANDLE event_log_handle()
    {
        if (!hEventLog_)
        {
            ...
            {
                STL_LOG_THROW(internal::win32_error("RegisterEventSource"));
            }
        }
...
protected:
    void sink_it_(const details::log_msg &msg) override
    {
        ...
        if (!succeeded)
        {
            STL_LOG_THROW(win32_error("ReportEvent"));
        }
    }

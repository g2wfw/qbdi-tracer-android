...
#    if !defined(STL_LOG_ANDROID_RETRIES)
#        define STL_LOG_ANDROID_RETRIES 2
#    endif
...
protected:
    void sink_it_(const details::log_msg &msg) override
    {
        ...
        int retry_count = 0;
        while ((ret == -11 /*EAGAIN*/) && (retry_count < STL_LOG_ANDROID_RETRIES))
        {
...
private:
    ...
    static android_LogPriority convert_to_android_(stl::level::level_enum level)
    {
        switch (level)
        {
        case stl::level::trace:
            return ANDROID_LOG_VERBOSE;
        case stl::level::debug:

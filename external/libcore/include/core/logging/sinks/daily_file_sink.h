...
struct daily_filename_calculator
{
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        return fmt_lib::format(STL_LOG_STL_FMT_STRING(STL_LOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}{}")), basename, now_tm.tm_year + 1900,
 ...
 * Usages:
 *    auto sink =  std::make_shared<stl::sinks::daily_file_format_sink_mt>("myapp-%Y-%m-%d:%H:%M:%S.log", hour, minute);"
 *    auto logger = stl::daily_logger_format_mt("loggername, "myapp-%Y-%m-%d:%X.log", hour,  minute)"
 *
 */
...
private:
    ...
    tm now_tm(log_clock::time_point tp)
    {
        ...
        return stl::details::os::localtime(tnow);
    }

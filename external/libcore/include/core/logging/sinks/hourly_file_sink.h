...
struct hourly_filename_calculator
{
    ...
    static filename_t calc_filename(const filename_t &filename, const tm &now_tm)
    {
        ...
        return fmt_lib::format(STL_LOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}_{:02d}{}"), basename, now_tm.tm_year + 1900, now_tm.tm_mon + 1,
    ...
    void delete_old_()
    {
        ...
        if (filenames_q_.full())
        {
            ...
            if (!ok)
            {
                ...
                STL_LOG_THROW(spdlog_ex("Failed removing hourly file " + filename_to_str(old_filename), errno));
            }
        }
        filenames_q_.push_back(std::move(current_file));
    }

...
STL_LOG_INLINE pattern_formatter::pattern_formatter(
...
STL_LOG_INLINE pattern_formatter::pattern_formatter(pattern_time_type time_type, std::string eol)
    : pattern_("%+")
    , eol_(std::move(eol))
    , pattern_time_type_(time_type)
...
STL_LOG_INLINE std::unique_ptr<formatter> pattern_formatter::clone() const
{
...
STL_LOG_INLINE void pattern_formatter::format(const details::log_msg &msg, memory_buf_t &dest)
{
    if (need_localtime_)
    {
...
STL_LOG_INLINE void pattern_formatter::set_pattern(std::string pattern)
{
    pattern_ = std::move(pattern);
    need_localtime_ = false;
    compile_pattern_(pattern_);
...
STL_LOG_INLINE void pattern_formatter::need_localtime(bool need)
{
    need_localtime_ = need;
}

STL_LOG_INLINE std::tm pattern_formatter::get_time_(const details::log_msg &msg)
{
...
STL_LOG_INLINE void pattern_formatter::handle_flag_(char flag, details::padding_info padding)
{
    ...
    default: // Unknown flag appears as is
        ...
        // stl::set_pattern("[%10!] %v") => "[      main] some message"
        // stl::set_pattern("[%3!!] %v") => "[mai] some message"
        else
        {
...
// Return padding.
STL_LOG_INLINE details::padding_info pattern_formatter::handle_padspec_(std::string::const_iterator &it, std::string::const_iterator end)
{
...
STL_LOG_INLINE void pattern_formatter::compile_pattern_(const std::string &pattern)
{

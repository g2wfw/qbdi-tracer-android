...
/**
    ...
    @tags{Core}
*/
    class STL_EXPORT StringRef final {
    public:
        ...
        */
        ...
        /** 从字符串创建StringRef。
        StringRef对象不会从std:：string中获取所有权或复制数据，
        因此，您必须确保源字符串对象在
        StringRef的生存期。
        */
        ALWAYS_INLINE StringRef(const std::string &string) : StringRef(string.c_str()) {};
        ...
        stl_wchar operator[](int index) const noexcept { return text[index]; }
...
#if STL_STRING_UTF_TYPE != 8 && !defined (DOXYGEN)
...
/** Case-sensitive comparison of two strings. */
    bool STL_CALLTYPE operator==(const String &string1, StringRef string2) noexcept;

...
*/
    ...
    public:
        ...
        */
        ...
        String join(const char *delim) const;
...
template<>
struct stl::fmt::v9::formatter<stl::StringArray> {
    constexpr auto parse(format_parse_context &ctx) { return ctx.begin(); }

    template<typename FormatContext>
    auto format(const stl::StringArray &data, FormatContext &ctx) {
        return fmt::v9::format_to(ctx.out(), "{}", data.join(","));
    }
};

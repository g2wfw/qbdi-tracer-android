...
STL_BEGIN_IGNORE_WARNINGS_MSVC (4702)

#ifndef DOXYGEN
#define STL_OPTIONAL_OPERATORS X(==) X(!=) X(<) X(<=) X(>) X(>=)
#endif
...
/**
    ...
    This is intended to stand-in for std::optional while STL's minimum
    ...
    the same way that stl::ScopedPointer was deprecated in favour of
    std::unique_ptr after C++11.

    This isn't really intended to be used by STL clients. Instead, it's to be
    used internally in STL code, with an API close-enough to std::optional

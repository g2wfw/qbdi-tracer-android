...
/** Using stl_hypot is easier than dealing with the different types of hypot function
    that are provided by the various platforms and compilers. */
template <typename Type>
Type stl_hypot (Type a, Type b) noexcept
{
   #if STL_MSVC
...
#ifndef DOXYGEN
template <>
inline float stl_hypot (float a, float b) noexcept
{
   #if STL_MSVC
    return _hypotf (a, b);
   #else
...
bool stl_isfinite (NumericType value) noexcept
{
...
{
    STL_BEGIN_IGNORE_WARNINGS_GCC_LIKE ("-Wfloat-equal")
    return a == b;
...
{
    if (! (stl_isfinite (a) && stl_isfinite (b)))
...
//==============================================================================
#if STL_INTEL || DOXYGEN
 /** This macro can be applied to a float variable to check whether it contains a denormalised
     value, and to normalise it if necessary.
     On CPUs that aren't vulnerable to denormalisation problems, this will have no effect.
 */
 #define STL_UNDENORMALISE(x)   { (x) += 0.1f; (x) -= 0.1f; }
#else
 #define STL_UNDENORMALISE(x)
#endif

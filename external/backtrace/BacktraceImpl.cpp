...
#include "include/backtrace/BacktraceMap.h"
...
bool Backtrace::VerifyReadWordArgs(uintptr_t ptr, word_t *out_value) {
    if (ptr & (sizeof(word_t) - 1)) {
        LOGW("invalid pointer %p", (void *) ptr);
        *out_value = (word_t) -1;
        return false;
    }
    return true;
...
std::string Backtrace::FormatFrameData(const backtrace_frame_data_t *frame) {
    ...
    if (!frame->func_name.empty() && frame->func_offset) {
        ...
        snprintf(buf, sizeof(buf), "#%02zu pc %0*" PRIxPTR "  %s %p ", frame->num,
                 (int) sizeof(uintptr_t) * 2, relative_pc, map_name,(void *)frame->pc);
    }
...
bool BacktraceCurrent::ReadWord(uintptr_t ptr, word_t *out_value) {
    ...
    if (map && map->flags & PROT_READ) {
        *out_value = *reinterpret_cast<word_t *>(ptr);
        ...
        LOGW("pointer %p not in a readable map", reinterpret_cast<void *>(ptr));
...
bool BacktracePtrace::ReadWord(uintptr_t ptr, word_t *out_value) {
    if (!VerifyReadWordArgs(ptr, out_value)) {
        ...
        LOGW("MacOS does not support reading from another pid.");
        return false;
#else
    ...
    *out_value = ptrace(PTRACE_PEEKTEXT, Tid(), reinterpret_cast<void *>(ptr), NULL);
    if (*out_value == static_cast<word_t>(-1) && errno) {
        LOGW("invalid pointer %p reading from tid %d, ptrace() strerror(errno)=%s",
...
Backtrace *Backtrace::CreateTarget(pid_t pid, pid_t tid, BacktraceMap *map) {
    return CreatePtraceObj(pid, pid, map);
}

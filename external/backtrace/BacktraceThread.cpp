...
void ThreadEntry::Wait(int value) {
  ...
  if (clock_gettime(CLOCK_MONOTONIC, &ts) == -1) {
    LOGW("clock_gettime failed: %s", strerror(errno));
    abort();
  }
  ts.tv_sec += 10;
  ...
  while (wait_value_ != value) {
    ...
    if (ret != 0) {
      LOGW("pthread_cond_timedwait failed: %s", strerror(ret));
      break;
    }
  }
...
static void SignalHandler(int, siginfo_t*, void* sigcontext) {
  ...
  if (!entry) {
    LOGW("Unable to find pid %d tid %d information", getpid(), gettid());
    return;
  }
...
bool BacktraceThread::Unwind(size_t num_ignore_frames, ucontext_t* ucontext) {
  ...
  if (pthread_mutex_lock(&g_sigaction_mutex) < 0) {
    LOGW("sigaction failed: %s", strerror(errno));
    return false;
  }
  ...
  if (tgkill(Pid(), Tid(), THREAD_SIGNAL) != 0) {
    LOGW("tgkill %d failed: %s", Tid(), strerror(errno));

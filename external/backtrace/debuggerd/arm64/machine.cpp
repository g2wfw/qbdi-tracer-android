...
void dump_memory_and_code( pid_t tid) {
    ...
    if (ptrace(PTRACE_GETREGSET, tid, (void*)NT_PRSTATUS, &io) == -1) {
        _LOG( logtype::ERROR, "%s: ptrace failed to get registers: %s\n",
    ...
    _LOG( logtype::MEMORY, "\ncode around pc:\n");
    dump_memory( tid, (uintptr_t)regs.pc);

    if (regs.pc != regs.sp) {
        _LOG(logtype::MEMORY, "\ncode around sp:\n");
        dump_memory( tid, (uintptr_t)regs.sp);
    }
}
...
void dump_registers( pid_t tid) {
  ...
  if (ptrace(PTRACE_GETREGSET, tid, (void*) NT_PRSTATUS, (void*) &io) == -1) {
    _LOG( logtype::ERROR, "ptrace error: %s\n", strerror(errno));
    return;
  }

  for (int i = 0; i < 28; i += 4) {
    _LOG( logtype::REGISTERS,
  ...
  _LOG( logtype::REGISTERS, "    x28  %016llx  x29  %016llx  x30  %016llx\n",
       r.regs[28], r.regs[29], r.regs[30]);

  _LOG( logtype::REGISTERS, "    sp   %016llx  pc   %016llx  pstate %016llx\n",
       r.sp, r.pc, r.pstate);
  ...
  _LOG( logtype::FP_REGISTERS, "    fpsr %08x  fpcr %08x\n", f.fpsr, f.fpcr);
}
